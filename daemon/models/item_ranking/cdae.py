...
from daemon.utils.evaluation.RankingMetrics import evaluate
...
class ICDAE():
    ...
    def build_network(self, hidden_neuron=500, corruption_level=0):
        self.corrupted_interact_matrix = tf.placeholder(dtype=tf.float32, shape=[None, self.num_user],
                                                        name="corrupted_interact_matrix")
        ...
        W_prime = tf.Variable(tf.random_normal([hidden_neuron, self.num_user], stddev=0.01, name="W_prime"))
        ...
        self.layer_2 = tf.sigmoid(tf.matmul(layer_1, W_prime) + b_prime, name="layer_2")
    ...
    def test(self):
        self.reconstruction = self.sess.run(self.layer_2, feed_dict={
            self.corrupted_interact_matrix: self.train_data,
            self.item_id: range(self.num_item)
        }).transpose()
